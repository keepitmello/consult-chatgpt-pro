...
- Keep that single profile and browser instance. Run independent consult queries concurrently with agbrowse `--parallel` in invocation-owned tabs; serialize only calls targeting the same saved provider session. Do not clone profiles, auto-start Chrome from `agbrowse`, or start parallel headed browsers.
