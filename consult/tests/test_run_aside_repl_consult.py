...
class AsideReplConsultTest(unittest.TestCase):
    ...
    def test_generated_script_has_quality_mapping_and_hard_deadline(self) -> None:
        ...
        self.assertEqual(MODULE.SUBMIT_TIMEOUT_SECONDS, 60)
        self.assertIn("var targetIndex = 4", xhigh)
        self.assertIn("var targetIndex = 5", pro)
        self.assertIn("pre-submit preparation exceeded 50 seconds", pro)
        self.assertIn("user turn committed after 60-second deadline", pro)
        self.assertIn("submitElapsedMs >= 60000", pro)
